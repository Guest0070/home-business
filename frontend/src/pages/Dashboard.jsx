...
export default function Dashboard({ user, setPage }) {
  ...
  const routePressure = (charts.routeMetrics || []).slice(0, 5);
  const exceptionRows = useMemo(() => [
    ...exceptions.activeHalts.map((row) => ({
      id: row.id,
      title: `${row.vehicle_no} halted`,
      meta: `${String(row.halt_type || '').replace('_', ' ')} / LR ${row.lr_number || '-'} / ${row.driver_name || '-'}`,
      value: new Date(row.started_at).toLocaleString('en-IN'),
      tone: 'danger',
      page: 'trips'
    })),
    ...exceptions.dispatchBlocks.map((row) => ({
      id: row.id,
      title: `${row.vehicle_no} dispatch ${row.severity}`,
      meta: `PUCC / permit next due ${row.next_due || '-'}`,
      value: row.severity === 'blocked' ? 'Halt' : 'Watch',
      tone: row.severity === 'blocked' ? 'danger' : 'warning',
      page: 'vehicles'
    })),
    ...exceptions.deliveryPressure.map((row) => ({
      id: row.id,
      title: `${row.do_number} needs dispatch attention`,
      meta: `${row.mine_name || '-'} to ${row.factory_name || '-'} / ${tons(row.pending_tons)} pending`,
      value: String(row.tracking_status || row.priority || '-').replace('_', ' '),
      tone: row.tracking_status === 'expired' ? 'danger' : 'warning',
      page: 'delivery-orders'
    })),
    ...exceptions.loanDues.map((row) => ({
      id: row.id,
      title: `${row.loan_name} installment due`,
      meta: `${row.lender_name || '-'} / ${row.account_name || '-'}`,
      value: money(row.amount),
      tone: row.status === 'overdue' ? 'danger' : 'warning',
      page: 'bank-loans'
    })),
    ...exceptions.complianceDue.map((row) => ({
      id: row.id,
      title: `${row.vehicle_no} ${String(row.document_type).replaceAll('_', ' ')}`,
      meta: `Due ${row.due_date} / ${row.days_left ?? '-'} days`,
      value: String(row.reminder_state || '').replace('_', ' '),
      tone: row.reminder_state === 'overdue' ? 'danger' : 'warning',
      page: 'compliance'
    }))
  ].slice(0, 10), [exceptions]);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
        ...
        <MetricCard label="Cost Per Ton" value={money(data?.cost_per_ton)} tone="danger" />
        <MetricCard label="Earnings Per KM" value={money(data?.earnings_per_km)} tone="money" />
      </div>
      ...
      <DashboardPanel
        ...
        subtitle="Stops, dispatch blockers, urgent D.O.s, loan dues, and expiring documents in one place."
        actionLabel="Open Reports"
        onAction={() => setPage('reports')}
      >
        {exceptionRows.length === 0 ? (
          <div className="dashboard-empty-note">No active exceptions right now.</div>
        ) : (
          <div className="dashboard-list dashboard-exception-list">
            {exceptionRows.map((row) => (
              <button key={`${row.page}-${row.id}`} className="dashboard-list-item dashboard-exception-row" onClick={() => setPage(row.page)}>
                <div>
                  <div className="dashboard-list-title">{row.title}</div>
                  <div className="dashboard-list-meta">{row.meta}</div>
                </div>
                <div className={row.tone === 'danger' ? 'dashboard-list-value text-red-700' : 'dashboard-list-value text-amber-700'}>
                  {row.value}
                </div>
              </button>
            ))}
      ...
      <div className="grid gap-4 xl:grid-cols-4">
        <DashboardPanel
          title="Route Cost Watch"
          subtitle="Routes where CPT and EPK need attention."
          actionLabel="Open Reports"
          onAction={() => setPage('reports')}
        >
          <SpotlightList
            rows={routePressure}
            empty="Route cost per ton will show once trips are recorded."
            renderTitle={(row) => `${row.mine_name} to ${row.factory_name}`}
            renderMeta={(row) => `${row.trips || 0} trips / ${tons(row.tons)} / EPK ${money(row.earnings_per_km)}`}
            renderValue={(row) => `CPT ${money(row.cost_per_ton)}`}
          />
        </DashboardPanel>
