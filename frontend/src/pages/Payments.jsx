...
const paymentTabs = [
  ...
  { id: 'reconciliation', label: 'Reconciliation', hint: 'Shortages' },
...
const tabMeta = {
  ...
  reconciliation: {
    title: 'Shortage Reconciliation',
    description: 'Import factory weighment sheets, match trips by LR or truck plus date, and create shortage deductions once.'
  }
};
...
export default function Payments() {
  ...
  async function reviewReconciliation(event) {
    ...
    try {
      setReconciliationPreview(await apiForm('/payments/reconciliation/preview', formData));
    } catch (err) {
      setError(err.message);
  ...
  async function confirmReconciliation() {
    ...
    setError('');
    try {
      setReconciliationSummary(await apiForm('/payments/reconciliation/import', formData));
      setReconciliationPreview(null);
      setReconciliationFile(null);
      await load();
      setTab('history');
    ...
    return (
      <DataTable
        ...
        columns={[
          ...
          { key: 'shortage_deductions', label: 'Shortage', render: (row) => money(row.shortage_deductions) },
          { key: 'net_billing', label: 'Net Billing', render: (row) => money(row.net_billing) },
    ...
    return (
      <DataTable
        ...
        columns={[
          ...
          { key: 'bank_account_name', label: 'Bank', render: (row) => row.bank_account_name || '-' },
          { key: 'amount', label: 'Amount', render: (row) => money(row.amount) },
          { key: 'payment_type', label: 'Type', render: (row) => String(row.payment_type || 'receipt').replaceAll('_', ' ') },
          { key: 'mode', label: 'Mode' },
          { key: 'reference_no', label: 'Reference', render: (row) => row.reference_no || '-' },
    ...
    return (
      <div className="grid gap-4 xl:grid-cols-[420px_1fr]">
        <section className="glass glass-card space-y-3 p-4">
          <h2 className="text-lg font-bold">Factory Shortage Import</h2>
          <p className="text-sm text-slate-600">Upload the factory weighment sheet. Matching uses LR first, then vehicle number plus trip date. Existing shortage deductions are skipped by LR + vehicle.</p>
          {reconciliationSummary && (
            <div className="rounded border border-teal-200 bg-teal-50 px-3 py-2 text-sm text-teal-800">
              Checked {reconciliationSummary.totalRows} rows. Created {reconciliationSummary.created}, skipped {reconciliationSummary.skipped}, duplicates {reconciliationSummary.duplicates}, failed {reconciliationSummary.failed}.
            </div>
          )}
          <button className="btn-muted w-full" onClick={() => downloadApiFile('/payments/reconciliation/template', 'shortage-reconciliation-template.xlsx').catch((err) => setError(err.message))}>Download Template</button>
          <label className="block">
            Review Reconciliation Excel
            <input className="mt-1" type="file" accept=".xlsx" onChange={reviewReconciliation} />
          </label>
        </section>

        <ImportReview
          title="Shortage Reconciliation Review"
