...
async function resolveVehicleForTrip(client, payload, existingTrip) {
  ...
  const complianceBlock = await client.query(
    `SELECT vehicle_no, document_type, due_date
     FROM compliance_reminders
     WHERE vehicle_id = $1
      AND status <> 'completed'
      AND document_type IN ('pollution_certificate', 'all_india_permit')
      AND due_date <= CURRENT_DATE + INTERVAL '2 days'
     ORDER BY due_date ASC
     LIMIT 1`,
    [vehicle.id]
  );
  if (complianceBlock.rows[0]) {
    const row = complianceBlock.rows[0];
    throw new ApiError(
      400,
      `${vehicle.vehicle_no} is blocked for new trips: ${String(row.document_type).replaceAll('_', ' ')} expires on ${row.due_date}`
    );
  }
  return vehicle;
}
