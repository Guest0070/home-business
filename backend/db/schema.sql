...
CREATE TABLE IF NOT EXISTS payments (
  ...
  payment_type VARCHAR(40) NOT NULL DEFAULT 'receipt',
...
ALTER TABLE payments ADD COLUMN IF NOT EXISTS trip_id UUID REFERENCES trips(id) ON DELETE SET NULL;
ALTER TABLE payments ADD COLUMN IF NOT EXISTS payment_type VARCHAR(40) NOT NULL DEFAULT 'receipt';
...
CREATE TABLE IF NOT EXISTS trip_halt_logs (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  trip_id UUID NOT NULL REFERENCES trips(id) ON DELETE CASCADE,
  halt_type VARCHAR(40) NOT NULL DEFAULT 'breakdown',
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_payments_shortage_reference ON payments(reference_no)
  WHERE payment_type = 'shortage_deduction' AND reference_no IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_payments_bank_account ON payments(bank_account_id);
CREATE INDEX IF NOT EXISTS idx_trip_halt_logs_trip ON trip_halt_logs(trip_id, started_at DESC);
...
deductions AS (
  SELECT factory_id, SUM(amount) AS shortage_deductions
  FROM payments
  WHERE payment_type = 'shortage_deduction'
  GROUP BY factory_id
)
SELECT
  f.id AS factory_id,
  ...
  COALESCE(d.shortage_deductions, 0) AS shortage_deductions,
  COALESCE(b.total_billing, 0) - COALESCE(d.shortage_deductions, 0) AS net_billing,
  COALESCE(r.payments_received, 0) AS payments_received,
  COALESCE(b.total_billing, 0) - COALESCE(d.shortage_deductions, 0) - COALESCE(r.payments_received, 0) AS pending_balance
FROM factories f
LEFT JOIN billing b ON b.factory_id = f.id
LEFT JOIN receipts r ON r.factory_id = f.id
LEFT JOIN deductions d ON d.factory_id = f.id;
